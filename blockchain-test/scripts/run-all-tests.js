...
const EXTENDED_SUITES = [
  { name:'Event Emission',         file:'test/events/event-emission.test.js',              category:'events'           },
  { name:'Static Analysis',        file:'test/static-analysis/static-analysis.test.js',    category:'static-analysis'  },
  { name:'E2E Full Lifecycle',     file:'test/e2e/full-lifecycle.test.js',                 category:'e2e'              },
];
...
function runSuite(suite, idx, total) {
  return new Promise(resolve => {
    ...
    const tag   = suite.category === 'events'           ? '🔔' :
                  suite.category === 'static-analysis'  ? '🔍' :
                  suite.category === 'e2e'              ? '🔄' : '▶';
    console.log(`\n${label} ${c('cyan', tag)} ${c('bold', suite.name)}`);
    console.log('     ' + '─'.repeat(53));
    const t0 = Date.now();
    exec(`node "${path.join(__dirname,'..', suite.file)}"`, { timeout: 480000 }, (err, stdout) => {
      const dur = ((Date.now()-t0)/1000).toFixed(1);
      if (stdout) process.stdout.write(stdout);
      if (err) {
        console.log(`     ${c('red','✗ FAILED')} (${dur}s)`);
        resolve({ ...suite, status:'FAILED', duration:dur, error:err.message.slice(0,200) });
      } else {
...
function generateHTML(report, htmlPath) {
  const categoryColors = {
    performance: '#2196F3', security: '#f44336', attacks: '#FF9800',
    scalability: '#9C27B0', events: '#00BCD4', 'static-analysis': '#607D8B', e2e: '#4CAF50'
  };
  const rows = report.suiteResults.map(r => {
  ...
  fs.writeFileSync(htmlPath, `<!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8">
...
async function main() {
  ...
  const onlyCat = args.find(a => a.startsWith('--only='))?.replace('--only=','');
  ...
  const report = {
    ...
    summary: { total: toRun.length, passed, failed, totalDurationSec: totalSec },
    suiteResults
  };
