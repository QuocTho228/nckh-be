...
function getWeb3() { return new Web3(GANACHE_URL); }
...
function loadABI(name) {
  ...
  throw new Error(`ABI không tìm thấy: ${name}.json  →  cd blockchain && truffle compile`);
}
...
async function createBatch(tc, from, name, producerId, quantity, productTypeId) {
  const now = BigInt(Math.floor(Date.now() / 1000));
  return tc.methods.createBatch(
    name,
    toBI(producerId),
...
async function approveBatch(tc, from, batchId, approverId = 1) {
  return tc.methods.approveBatch(toBI(batchId), toBI(approverId))
    .send({ from, gas: 500000n });
}

async function rejectBatch(tc, from, batchId, approverId = 1, reason = 'Rejected') {
  return tc.methods.rejectBatch(toBI(batchId), toBI(approverId), reason)
    .send({ from, gas: 500000n });
}

async function recordPurchase(tc, from, batchId, purchaserId = 1, qty = 500, price = 50000) {
  return tc.methods.recordPurchase(toBI(batchId), toBI(purchaserId), toBI(qty), toBI(price))
    .send({ from, gas: 500000n });
}

// actionCode: 0=start, 1=checkpoint, 2=delay, 3=delivered
async function updateTransport(tc, from, batchId, participantId = 1, actionCode = 0) {
  return tc.methods.updateTransportStatus(
    toBI(batchId), toBI(participantId), actionCode, 28, 75
  ).send({ from, gas: 500000n });
}

async function recordProcessing(tc, from, batchId, processorId = 1) {
  return tc.methods.recordProcessing(
    toBI(batchId), toBI(processorId), 0, 500n, 480n
  ).send({ from, gas: 500000n });
}

async function recordQualityTest(tc, from, batchId, inspectorId = 1, passed = true) {
  return tc.methods.recordQualityTest(toBI(batchId), toBI(inspectorId), passed)
    .send({ from, gas: 500000n });
}

async function warehouseConfirmation(tc, from, batchId, warehouseId = 1) {
  return tc.methods.warehouseConfirmation(toBI(batchId), toBI(warehouseId))
    .send({ from, gas: 300000n });
}

async function registerTree(al, from, qrCode, farmerId = 1, regionId = 1) {
  return al.methods.registerTree(qrCode, toBI(farmerId), toBI(regionId), 'Musang King', 'Premium', '10.45,106.32')
    .send({ from, gas: 600000n });
}

async function addActivityLog(al, from, batchId, actName, desc, participantId = 1) {
  return al.methods.addActivityLog(toBI(batchId), toBI(participantId), actName, desc, false)
    .send({ from, gas: 300000n });
}

async function addDetailedLog(al, from, batchId, cat, actName, desc, participantId = 1, isSystem = false) {
  return al.methods.addDetailedActivityLog(toBI(batchId), toBI(participantId), toBI(cat), actName, desc, isSystem)
    .send({ from, gas: 300000n });
}

// ─── Stats helper (BigInt-safe) ───────────────────────────────────────────
function calcStats(times) {
  if (!times.length) return { min:0, max:0, avg:0, median:0, p95:0, count:0 };
...
async function increaseTime(web3, seconds) {
  ...
  try {
    await web3.currentProvider.request({ method:'evm_increaseTime', params:[seconds] });
...
module.exports = {
  GANACHE_URL, getWeb3, loadABI, deployContracts,
  createBatch, approveBatch, rejectBatch, recordPurchase,
  updateTransport, recordProcessing, recordQualityTest,
  warehouseConfirmation, registerTree, addActivityLog, addDetailedLog,
  calcStats, now, increaseTime, getBatchId, sleep, toN, toBI, sumBI,
};
